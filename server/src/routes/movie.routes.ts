...
movieRouter.get('/getMovie', async (req, res) => {
    ...
    if (typeof id === 'number') {
        let includeSimilarMovies = req.query.includeSimilarMovies === 'false' ? false : true;
        let formattedMovieResponse = {};

        let selectedMovie = await getMovie(id);
        //the get similar movies function only looks for simalr movies in the movies array. 
        // if(movies.find((movie) => movie.id === id) === undefined){
        //     includeSimilarMovies = false;
        // };

        if (includeSimilarMovies) {
            const similarMovies = getSimilarMovies(id, movies, selectedMovie);
