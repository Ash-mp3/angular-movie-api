export default function getSimilarMovies(id: number, movies: any[], currentMovie: any) {
    let similarMovies: any[] = [];
    
    //took this out because if you search a movie that isent in the movies array it throws an error
    // const selectedMovie = movies.find((movie) => movie.id === id);
    
    //changed to grab the currentMovies genre ids
    const selectedMovieGenres: any = currentMovie.genre_ids;
    ...
    movies.forEach((movie) => {
        selectedMovieGenres.forEach((genre: any) => {
            if (movie.genre_ids.indexOf(genre) !== -1 && movie.id !== id) {
